...
![GitHub stars](https://img.shields.io/github/stars/LearnPrompt/ai-news-radar?style=flat-square&color=f5c542)
![GitHub Pages](https://img.shields.io/badge/GitHub%20Pages-Live-green?style=flat-square)
![Actions](https://img.shields.io/github/actions/workflow/status/LearnPrompt/ai-news-radar/update-news.yml?branch=master&label=update&style=flat-square)
![Agent Skills](https://img.shields.io/badge/Agent%20Skills-ai--radar%20%2B%20%E4%BC%AF%E4%B9%90-blueviolet?style=flat-square)
![License](https://img.shields.io/badge/license-MIT-green.svg?style=flat-square)
在线页面 · English · 雷达Skill · 伯乐Skill · 信息源策略
</div>
---
30秒选边上车
...
本地复刻与个人部署记录
> 记录日期：2026-07-09  
> 当前用途：先保持 AI News Radar 原版稳定运行，暂不做品牌营销/涂料行业改造。
本地运行
本地项目目录：`C:\codex_projects_by_dd\ai-news-radar`
Python 虚拟环境：`.venv`
数据目录：`data/`
本地访问：`http://localhost:8080`
已验证：
可以本地运行 `scripts/update_news.py` 生成 24 小时 AI 新闻数据。
PC 浏览器可以打开本地页面。
手机在同一局域网下可以访问电脑本地服务。
页面可以看到新闻卡片、更新时间、来源状态、搜索和筛选等基础功能。
个人 GitHub Pages 部署
个人仓库：`https://github.com/xiaozhiyun/ai-news-radar`
Pages 地址：`https://xiaozhiyun.github.io/ai-news-radar/`
Pages 部署方式：`main` 分支 `/root`
GitHub 仓库需要的关键配置：
Actions 权限允许写入仓库内容，用于自动提交更新后的 `data/`。
Repository Secret：`FEISHU_BOT_WEBHOOK`
Actions Variable：`PUBLIC_SITE_URL`
注意：`FEISHU_BOT_WEBHOOK` 只保存在 GitHub Secret 中，不写入代码、README、日志或 Obsidian 笔记。
飞书日报推送
飞书群：`AI 日报雷达`
工作流：`Send Feishu AI Daily`
定时目标：每天北京时间 9:00 推送 AI 日报。
GitHub Actions 的 `cron` 使用 UTC；北京时间 09:00 对应 UTC 01:00。
当前定时配置为 `0,10,20,30,40,50 1 * * *`，只在 UTC 01 点（北京时间 09 点）窗口内多次尝试。
工作流内置北京时间守门：只有自动触发且运行时仍处于北京时间 09 点小时内才发送；如果 GitHub 延迟到晚上才启动，会直接跳过，避免晚间误推。
自动推送会写入 `data/feishu-daily-state.json` 记录当天是否已发送，防止 09 点窗口内重复推送；手动触发仍保留，用于测试和补发。
已验证：
GitHub Actions 手动运行后，可以生成最新数据。
飞书机器人可以收到 AI News Radar 日报。
日报中包含更新时间、过去 24 小时 AI 信号数量、来源状态、今日重点和 Pages 链接。
当前边界
本轮只做原版复刻、部署和日报推送验证，不接入涂料行业竞品，不接入立邦/三棵树/多乐士/嘉宝莉，不接入抖音/小红书/微信/B站，也不改造业务逻辑。
后续如果要做“品牌营销/涂料行业竞品情报雷达”，建议单独开新阶段，先定义信源、关键词、竞品名单、日报格式和数据边界，再开始改造。
